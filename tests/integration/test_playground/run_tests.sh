#!/bin/bash
# Final integration test script for vit commands
...
print_section "Stage 1: Initialize & Verify .vitignore"

print_step "1.1: Initialize vit repository"
uv run --directory "$MEMOV_ROOT" vit init --loc "$PROJECT_DIR"
print_success "Initialized"

print_step "1.2: Check .vitignore content"
cat .vitignore
print_success ".vitignore created with default rules"

print_step "1.3: Check status - hidden files should NOT appear"
uv run --directory "$MEMOV_ROOT" vit status --loc "$PROJECT_DIR"
...
OUTPUT=$(uv run --directory "$MEMOV_ROOT" vit track --loc "$PROJECT_DIR" \
...
uv run --directory "$MEMOV_ROOT" vit snap --loc "$PROJECT_DIR" -p "After modifications" -r "Snapshot changes"
...
print_section "Stage 4: Dynamic .vitignore Rules"
...
uv run --directory "$MEMOV_ROOT" vit status --loc "$PROJECT_DIR" | grep -E "(app.log|debug.log)" || echo "Files appear in status"
print_success "Log files visible before adding rule"

print_step "4.3: Add new ignore rules to .vitignore"
cat >> .vitignore << 'EOF'
...
print_step "4.5: Snapshot .vitignore changes"
uv run --directory "$MEMOV_ROOT" vit snap --loc "$PROJECT_DIR" -p "Updated ignore rules" -r "Added temp and log patterns"
print_success "Snapshotted .vitignore"
...
STATUS=$(uv run --directory "$MEMOV_ROOT" vit status --loc "$PROJECT_DIR" 2>&1)
...
print_step "5.3: Verify Modified state - .vitignore was changed"
echo "$STATUS" | grep "Modified:.*\.vitignore" > /dev/null && print_success ".vitignore is Modified"

print_step "5.4: Track an untracked file - state transition: Untracked → Clean"
uv run --directory "$MEMOV_ROOT" vit track --loc "$PROJECT_DIR" "$PROJECT_DIR/docs/api.md" -p "Track docs" -r "Adding API documentation"
STATUS=$(uv run --directory "$MEMOV_ROOT" vit status --loc "$PROJECT_DIR" 2>&1)
...
uv run --directory "$MEMOV_ROOT" vit snap --loc "$PROJECT_DIR" -p "Update docs" -r "Saved changes"
STATUS=$(uv run --directory "$MEMOV_ROOT" vit status --loc "$PROJECT_DIR" 2>&1)
echo "$STATUS" | grep "Clean:.*docs/api.md" > /dev/null && print_success "docs/api.md transitioned back to Clean"
echo "$STATUS" | grep "Clean:.*\.vitignore" > /dev/null && print_success ".vitignore also transitioned to Clean"
...
uv run --directory "$MEMOV_ROOT" vit snap --loc "$PROJECT_DIR" --files "$PROJECT_DIR/another.txt" -p "Fine-grained snap" -r "Only snapping another.txt"
STATUS=$(uv run --directory "$MEMOV_ROOT" vit status --loc "$PROJECT_DIR" 2>&1)
...
LAST_COMMIT=$(uv run --directory "$MEMOV_ROOT" vit history --loc "$PROJECT_DIR" 2>&1 | grep -oE '[a-f0-9]{7}' | head -1)
SHOW_OUTPUT=$(uv run --directory "$MEMOV_ROOT" vit show --loc "$PROJECT_DIR" "$LAST_COMMIT" 2>&1)
...
OUTPUT=$(uv run --directory "$MEMOV_ROOT" vit track --loc "$PROJECT_DIR" "$PROJECT_DIR/data/config.json" "$PROJECT_DIR/data/settings.yaml" -p "Data files" -r "JSON and YAML data" 2>&1)
check_output_for_errors "$OUTPUT"
...
uv run --directory "$MEMOV_ROOT" vit rename --loc "$PROJECT_DIR" "$PROJECT_DIR/normal.txt" "$PROJECT_DIR/renamed.txt" -p "Rename operation" -r "Renamed normal.txt"
...
uv run --directory "$MEMOV_ROOT" vit snap --loc "$PROJECT_DIR" -p "After rename" -r "Snapshotted rename"
print_success "Snapshot created"

print_section "Stage 8: History & Verification"

print_step "8.1: View complete history"
OUTPUT=$(uv run --directory "$MEMOV_ROOT" vit history --loc "$PROJECT_DIR" 2>&1)
check_output_for_errors "$OUTPUT"
echo "$OUTPUT"
print_success "History displayed"

print_step "8.2: Get first commit hash and show details"
FIRST_COMMIT=$(uv run --directory "$MEMOV_ROOT" vit history --loc "$PROJECT_DIR" 2>&1 | grep -oE '[a-f0-9]{7}' | tail -1)
if [ -z "$FIRST_COMMIT" ]; then
    print_error "Failed to get commit hash"
    exit 1
fi
OUTPUT=$(uv run --directory "$MEMOV_ROOT" vit show --loc "$PROJECT_DIR" "$FIRST_COMMIT" 2>&1)
check_output_for_errors "$OUTPUT"
echo "$OUTPUT"
print_success "Show command works"

print_step "8.3: Amend commit message"
OUTPUT=$(uv run --directory "$MEMOV_ROOT" vit amend --loc "$PROJECT_DIR" "$FIRST_COMMIT" -p "Updated prompt" -r "Updated response" --by_user 2>&1)
check_output_for_errors "$OUTPUT"
